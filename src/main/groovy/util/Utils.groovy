...
class Utils {
    ...
    // 获得当前时间
    static String getCurentTime(String formatter) {
        Date d = new Date();
...
//                        println 'leefnode.name:' + leefnode.name()
    ...
    public static String getUrlTextContent(String rootDir, String url, Map<String, String> errorMap) {
        try {
            ...
            println "IOException when request" + url
            iex.printStackTrace()
            errorMap.put("err_code", "ioexception");
            errorMap.put("err_msg", "" + iex.getMessage());
            return null
        } catch (Throwable ex) {
            ...
            //errorLinks.add(link)
            errorMap.put("err_code", "exception");
            errorMap.put("err_msg", "" + ex.getMessage());
            return null
        }
    }
